...
class EUCB(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=3, stride=1, activation='relu'):
        ...
        self.sx1 = nn.Sequential(
            nn.Upsample(scale_factor=2),
            nn.Conv2d(self.in_channels, self.in_channels, kernel_size=kernel_size, stride=stride, padding=kernel_size//2, groups=self.in_channels, bias=False),
	        
            
        )
        self.sx2 = nn.Sequential(
            nn.Upsample(scale_factor=2),
            nn.Conv2d(self.in_channels, self.in_channels, kernel_size=kernel_size, dilation=2, padding=(kernel_size // 2) * 2,
        ...
        self.init_weights('normal')
        self.norm_s = LayerNorm(self.in_channels, eps=1e-6, data_format="channels_first")
        self.act = nn.GELU()
    def init_weights(self, scheme=''):
        named_apply(partial(_init_weights, scheme=scheme), self)
